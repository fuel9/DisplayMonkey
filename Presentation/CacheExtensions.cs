...
namespace DisplayMonkey
{
    ...
    public static class CacheExtensions
    {
        ...
        private class CacheItemBase
        {
            public Guid Guid { get; protected set; }
            public uint Crc32 { get; protected set; }
        }

        private class CacheItem<T> : CacheItemBase
        {
            public CacheItem(T _data)
            {
                this.Data = _data;
                ...
                if (_data is byte[])
                {
                    this.Crc32 = CacheExtensions.crc32(_data as byte[], 4098);  // limit to the first memory page for performance
                }

                //Debug.Print(string.Format("          --> crc={0}", this.Crc32));
            }
        ...
        // source: http://stackoverflow.com/questions/21175917/how-can-i-calculate-a-crc32-as-a-signed-integer-in-c
        private static uint crc32(byte[] input, int length = 0)
        {
            uint[] table = new uint[]
            {
            ...
            unchecked
            {
                ...
                for (int i = 0, len = length <= 0 ? input.Length : Math.Min(length, input.Length); i < len; i++)
                {
                    crc = (crc >> 8) ^ table[
