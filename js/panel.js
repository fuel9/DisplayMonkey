...
var Canvas = Class.create({
	initialize: function (options) {
		...
		this.offsetMilliseconds = clientTime.diff(serverTime);
...
Ajax.PanelUpdaterBase = Class.create(Ajax.Base, {
	...
	_onGetNextFrame: function () {
		// get next frame
		var p = $H({ panel: this.panelId, frame: this.currentId, feature: this.featureId });
		//alert(p.inspect());
...
var Clock = Class.create(PeriodicalExecuter, {
	initialize: function ($super, id) {
		var div = $(this.containerId = id);
		this.showDate = div.readAttribute("data-show-date");
		this.showTime = div.readAttribute("data-show-time");
		$super(this.callBack, 1);
	}
